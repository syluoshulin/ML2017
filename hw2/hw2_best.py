...
for it in range(epoch):
	...
	for itn in range(int(trX_row/batch_size)):
		...
		par_weight_d = -1*rawdata_trX[shuffle_d[itn]*batch_size:(shuffle_d[itn]+1)*batch_size,:].T.dot(diff_y_fx) + 2*regulation_d*weight_d
		par_bias_d = -1*np.sum(diff_y_fx) + 2*regulation_d*bias_d
		hist_par_weight_d = np.sqrt(hist_par_weight_d**2+par_weight_d**2)
		hist_par_bias_d = np.sqrt(hist_par_bias_d**2+par_bias_d**2)
		if(it<10):
			weight_d -= lrate_d*par_weight_d
			bias_d -= lrate_d*par_bias_d
		else:
			weight_d -= lrate_d*par_weight_d/hist_par_weight_d
			bias_d -= lrate_d*par_bias_d/hist_par_bias_d
	if(it%1==0):
